                            ...
                            const AVFrame *frame, int *got_packet)
{
    ...
    if (frame) {
        ...
        if (need_copy) {
            if (frame->data[3] && frame->buf[0]) {
                // Evil evil evil hack
                struct OMXOutputContext* octx = av_buffer_get_opaque(frame->buf[0]);
                void* addr = &((OMX_COMPONENTTYPE*)s->handle)->ComponentRoleEnum;
                typedef OMX_ERRORTYPE (*CopyRoutineType)(
                        OMX_IN OMX_HANDLETYPE hComponent,
                        OMX_IN OMX_BUFFERHEADERTYPE* dst_buf,
                        OMX_IN OMX_BUFFERHEADERTYPE* src_buf,
                        OMX_IN OMX_U32 size);
                CopyRoutineType func = *(CopyRoutineType*)(void*)((void**)addr + 4);
                func(s->handle, buffer, octx->buffer, buffer->nFilledLen);
            } else {
